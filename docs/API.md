...
- `PUT` endpoints accept **partial updates** â€” only send the fields you want
  to change.
...
## Deals â€” `/api/deals` (Sales Pipeline)
...
`meeting` â€” combined with `related_id`, it links the task to any record.
...
## AI Endpoints â€” `/api/ai`

These power the AI features described in the README. They all return `200`
even when AI is not configured â€” check the `ai_enabled` field in the
