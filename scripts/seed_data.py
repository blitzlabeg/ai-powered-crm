...
def run() -> None:
 ...
 lead = Lead(
 title=f"{company.name} ” expansion opportunity #{i + 1}",
 ...
 contact = random.choice([c for c in contacts if c.company_id == company.id])
 deal = Deal(
 title=f"{company.name} ” {stage.value.replace('_', ' ').title()} Deal",
 stage=stage,
 ...
 db.add(
 Note(
 body="Initial discovery call went well ” strong interest in the Pro plan.",
 author="Jordan Lee",
 ...
 print(" œ Seed data created successfully.")
